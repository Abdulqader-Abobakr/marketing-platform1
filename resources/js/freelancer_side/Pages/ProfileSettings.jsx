...
export default function ProfileSettings({ pageTitle, user = {} }) {
    const [notifications, setNotifications] = useState(() => {
        try {
            return localStorage.getItem('freelancer_notifications') !== 'false';
        } catch {
            return true;
        }
    });
    const [twoFactor, setTwoFactor] = useState(() => {
        try {
            return localStorage.getItem('freelancer_two_factor') === 'true';
        } catch {
            return false;
        }
    });
    ...
    useEffect(() => {
        localStorage.setItem('freelancer_notifications', String(notifications));
    }, [notifications]);

    useEffect(() => {
        localStorage.setItem('freelancer_two_factor', String(twoFactor));
    }, [twoFactor]);
        ...
        <div className="p-8 space-y-8" dir="rtl">
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <section className="bg-linear-to-r from-primary/5 to-primary/10 rounded-2xl border border-primary/10 p-8">
                    ...
                    <p className="text-sm text-slate-600">يمكنك تحديث بياناتك بشكل دوري لتظهر خدماتك المهنية بشكل واضح للعملاء.</p>
                </section>
            </div>
        </div>
    );
